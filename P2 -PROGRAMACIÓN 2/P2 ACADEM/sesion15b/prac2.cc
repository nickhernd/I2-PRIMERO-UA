...
// guarda el Platform a un fichero binario.
void saveData(const Platform &platform){
	...
	if(fich.is_open()){
		...
		for(Subscriber s : platform.subscribers){
			// fichero <= BinSubscriber <= Subscriber
			bs.id = s.id;
...
void loadData(Platform &platform, string filename){
	...
	fich.open(filename.c_str(), ios::binary);
	if(fich.is_open()){
		// eliminamos todos los que hay actualmente
		platform.subscribers.clear();

		fich.read((char *) &bff, sizeof(bff));
		platform.name = bff.name; // string <= char []
...
// fichero => BinSubscriber => Subscriber => vector<Subscriber>
	...
	if(!validateName(sub.name)){
		...
		if(!validateEmailCool(sub.email)){
			...
			if(!validateIp(sub.mainIp)){
				...
				}while(buffy.eof() == false && okIp == true);
				if(okIp){
...
void importFromCsv(Platform &platform) {
	...
	cout << "Enter filename: ";
	getline(cin, filename);
	importFromCsv(platform, filename);
}
...
prac2	argc = 1
  ...
  0    1    2
prac2 -i data.csv	argc = 3 	argv[1] == "-i"

prac2 -l data.bin	argc = 3	argv[1] = "-l"

prac2 -i data.csv -l data.bin	argc = 5	argv[1] = "-i"
						argv[3] = "-l"

prac2 -l data.bin -i data.csv	argc = 5	argv[1] = "-l"
						argv[3] = "-i"
*/
bool leerArgumentos(int argc, char *argv[], string &txtName, string &binName){
	bool okArgs = true;
	...
	if(argc == 1){
		...
		if(argc == 3 && strcmp(argv[1], "-i") == 0){
			...
			if(argc == 3 && strcmp(argv[1], "-l") == 0){
				...
				if(argc == 5 && strcmp(argv[1], "-i") == 0 && strcmp(argv[3], "-l") == 0){
						binName = argv[4];
						txtName = argv[2];
						okArgs = true;
				}
				else{
					if(argc == 5 && strcmp(argv[1], "-l") == 0 && strcmp(argv[3], "-i") == 0){
						binName = argv[2];
						txtName = argv[4];
						okArgs = true;
					}
...
int main(int argc, char *argv[]) {
  bool okArgs;
  string textName, binName;

  Platform platform;
  platform.name = "Streamflix";
  platform.nextId = 1;

  okArgs = leerArgumentos(argc, argv, textName, binName);
  if(okArgs){
	  if(binName != ""){
	  	  loadData(platform, binName);   
      }
	  if(textName != ""){
          importFromCsv(platform, textName);
	  }
	  char option;
	  do {
	    showMainMenu();
